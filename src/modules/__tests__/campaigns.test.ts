  ...
  describe("list", () => {
    it("kampanyaları başarıyla listeler", async () => {
      ...
      if (campaigns.length > 0) {
        ...
        expect(campaign.budget).toHaveProperty("amount");
      }
    }, 10000);
  });
  ...
  describe("create and update", () => {
    ...
    it("yeni kampanya oluşturur", async () => {
      const response = await sdk.campaigns
        .create(testProfileId, { campaigns: [campaignData] })
        .catch((err) => {
          throw err;
        });
      ...
      expect(response[0]).toHaveProperty("campaignId");
      expect(response[0]).toHaveProperty("code", "SUCCESS");

      // Test için campaign ID'yi sakla
      testCampaignId = response[0].campaignId;
    }, 15000);
    ...
    it("kampanyayı günceller", async () => {
      ...
      const response = await sdk.campaigns
        .update(testProfileId, { campaigns: [updateData] })
        .catch((err) => {
          throw err;
        });
      ...
      // Response kontrolü
      expect(Array.isArray(response)).toBe(true);
      expect(response[0]).toHaveProperty("campaignId", testCampaignId);
      expect(response[0]).toHaveProperty("code", "SUCCESS");
    }, 15000);
